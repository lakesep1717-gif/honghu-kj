...
// 读取整合后的场景数据
const scenarios = JSON.parse(fs.readFileSync('C:/Users/10540/.qclaw/workspace/rpa-scenarios/scenarios_merged.json', 'utf8'));
...
const PLATFORMS = ["全部", "通用", "亚马逊", "Temu", "Shein", "沃尔玛", "TikTok Shop", "eBay", "速卖通", "Shopee", "Lazada", "Allegro", "OZON", "美客多", "独立站"];
...
.tag-platform-common { background: #fff3e0; color: #e65100; padding: 2px 8px; border-radius: 4px; font-size: 12px; margin-right: 6px; }
...
.platform-count { font-size: 11px; color: #999; margin-top: 4px; }
...
<div class="stat-grid">
  ...
  <div class="stat-card">
    <div class="num" id="statCommon">${scenarios.filter(s => s.platform === '通用').length}</div>
    <div class="label">通用场景</div>
  </div>
  <div class="stat-card">
...
function render() {
  let filtered = SCENARIOS.filter(s => {
    ...
    if (searchKeyword) {
      ...
      const plats = Array.isArray(s.platforms) ? s.platforms.join(' ') : '';
      const hay = [s.name, s.desc, s.platform, s.model, s.dept, steps, plats];
  ...
  document.getElementById('statCommon').textContent = filtered.filter(s => s.platform === '通用').length;
...
function renderCard(s, i) {
  ...
  const platClass = s.platform === '通用' ? 'tag-platform-common' : 'tag-platform';
  const steps = Array.isArray(s.sop) ? s.sop : [s.sop];
  const sopId = 'sop-' + i;
  const platInfo = s.platform === '通用' && s.platformCount ? '适用' + s.platformCount + '平台' : '';

  return '<div class="scenario-card" id="card-' + i + '">' +
    '<div class="card-header">' +
      '<div class="card-title">' +
        '<span class="' + platClass + '">' + esc(s.platform) + '</span>' +
        '<span class="tag-model">' + esc(s.model) + '</span>' +
        '<span class="' + typeClass + '">' + typeText + '</span>' +
        '<h3>' + esc(s.name) + '</h3>' +
        (platInfo ? '<div class="platform-count">' + platInfo + '</div>' : '') +
      '</div>' +
...
console.log('通用场景:', scenarios.filter(s => s.platform === '通用').length);
console.log('平台特定:', scenarios.filter(s => s.platform !== '通用').length);
