...
// Get SCENARIOS content (third occurrence, 216 objects, properly formatted)
var s2 = 61529;
var end = 122198;
var scenContent = sc.substring(s2 + 'const SCENARIOS='.length, end + 1);
console.log('SCENARIOS:', scenContent.length, 'chars,', (scenContent.match(/{name:/g)||[]).length, 'objects');

// Write the COMPLETE clean script (no file content, just a known-good structure)
var cleanScript = `let activeDept="全部", activePlatform="全部", activeModel="全部";
let searchKeyword="";
...
function renderCard(s,i){
  ...
  const steps=(typeof s.sop==='string'?s.sop.split('\\n'):s.sop);
  const sopHtml=steps.map((step,idx)=>\`<div class="sop-step"><div class="sop-step-num">\${idx+1}</div><div class="sop-step-text">\${step}</div></div>\`).join('');
...
try {
    ...
    console.log('Clean script: OK ✅');
} catch(e) {
    console.log('Clean script ERROR:', e.message);
    process.exit(1);
}

// Now build HTML with all parts
// Get HTML structure (before and after script)
var si = h.indexOf('<script>');
var ei = h.indexOf('</script>');
var htmlStart = h.substring(0, si);
var htmlEnd = h.substring(ei);

// Get DEPTS/PLATFORMS/MODELS from first occurrence
...
console.log('Full script:', fullScript.length, 'chars');
