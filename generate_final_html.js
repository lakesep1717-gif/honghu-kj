...
const data = JSON.parse(fs.readFileSync(path.join(__dirname, 'scenarios_cleaned.json'), 'utf8'));
...
<body>
    <div class="container">
        <div class="header">
            ...
            <div class="stats">
                <div class="stat-item">
                    <div class="stat-number" id="totalCount">${data.length}</div>
                    <div class="stat-label">总场景数</div>
                </div>
                <div class="stat-item">
                    <div class="stat-number">${[...new Set(data.map(s => s.platform))].length}</div>
...
${[...new Set(data.map(s => s.platform))].sort().map(p => `                <option value="${p}">${p}</option>`).join('\n')}
            </select>
...
${[...new Set(data.map(s => s.dept))].sort().map(d => `                <option value="${d}">${d}</option>`).join('\n')}
    ...
    <script>
        ...
        function renderScenarios(scenarios) {
            ...
            grid.innerHTML = scenarios.map((s, i) => '<div class="scenario-card"><div class="scenario-header"><div class="scenario-name">' + s.name + '</div><span class="scenario-type type-' + s.type + '">' + (s.type === 'fast' ? '快速' : '潜力') + '</span></div><div class="scenario-desc">' + s.desc + '</div><div class="scenario-meta"><span class="meta-tag">' + s.dept + '</span><span class="meta-tag">' + s.model + '</span></div>' + (s.platforms && s.platforms.length > 0 ? '<div class="platforms">' + s.platforms.map(p => '<span class="platform-tag">' + p + '</span>').join('') + '</div>' : '') + '<div class="sop-section">' + (s.sop && s.sop.length > 0 ? '<button class="sop-toggle" onclick="toggleSOP(\\'sop-' + i + '\\')">查看SOP</button><div class="sop-content" id="sop-' + i + '">' + s.sop.map((step, idx) => '<div class="sop-step"><div class="step-number">' + (idx + 1) + '</div><div class="step-text">' + step + '</div></div>').join('') + '</div>' : '<div class="sop-empty">SOP待补充</div>') + '</div></div>').join('');
        }
